...
class ProgramRoleWidgetTests(TestCase):
    ...
    def test_member_and_expert_cannot_access_manager_api(self):
        url = reverse("partner_programs:manager-overview", kwargs={"pk": self.program.pk})
    ...
    def test_leader_and_collaborator_see_identical_program_scoped_project(self):
        ...
        # Existing public legacy contract stays leader-only.
        detail = self.client.get(f"/programs/{self.program.pk}/")
        self.assertEqual(detail.status_code, 200)
        self.assertIsNone(detail.data["current_application"])
        self.client.force_authenticate(self.member)
        detail = self.client.get(f"/programs/{self.program.pk}/")
        self.assertEqual(detail.data["current_application"]["program_link_id"], link.pk)
    ...
    def test_distributed_stages_zero_partial_complete_and_not_ready(self):
        ...
        # Program creation adds a Comment criterion; isolate two explicit criteria here.
    ...
    def test_manager_metrics_match_overview_including_draft_team_and_other_program(self):
        ...
        overview = self.client.get(
            reverse("partner_programs:manager-overview", kwargs={"pk": self.program.pk})
        ).data
