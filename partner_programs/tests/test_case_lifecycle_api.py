...
from partner_programs.services.project_apply import _validate_program_field_ownership
...
class ProgramCaseLifecycleAPITests(TestCase):
    ...
    def test_apply_revalidates_choice_after_options_change_before_transaction(self):
        def change_options(**kwargs):
            _validate_program_field_ownership(**kwargs)
            self.field.options = "A|C"
            self.field.save()

        with patch(
            "partner_programs.services.project_apply._validate_program_field_ownership",
            side_effect=change_options,
        ):
            response = self.apply([{"field_id": self.field.pk, "value_text": "B"}])
