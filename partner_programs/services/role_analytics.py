...
def organizer_widget_metrics(program):
    """Счётчики legacy project-analytics без загрузки проектов и назначений.

    На PROD общий селектор уже выделен из React manager-overview. Считаем
    связи программы, а отсутствие команды берём из его списка внимания:
    он учитывает и лидеров, и Collaborator, включая связанные черновики.
    """
    participants = _participant_metrics(program.pk)
    solutions = _annotated_solution_rows(program).aggregate(
        created=Count("pk"), submitted=Count("pk", filter=Q(submitted=True))
    )
    return {
...
def participant_widget_metrics(program, user):
    ...
    if link_id is None:
        return result
    link = _annotated_solution_rows(program).select_related("project").get(pk=link_id)
...
def build_program_role_widget(program, user):
    """Возвращает только разрешённую ветвь роли текущего пользователя в программе."""
