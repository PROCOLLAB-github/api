...
Поле `current_application` присутствует в ответе всегда. Для анонимного
пользователя, не-участника и участника без заявки оно равно `null`. Для
участника с заявкой контракт выглядит так:
...
Заявка определяется строго для пары «текущий пользователь + текущая программа»
через `PartnerProgramProject`, причем владельцем считается только лидер
`Project`. Поле `submitted` передается напрямую из
`PartnerProgramProject.submitted`. Выборка не зависит от legacy-поля
`partner_program` в serializers проекта и не использует участие пользователя
в чужом проекте как collaborator. Если legacy-данные содержат несколько заявок
одного лидера в программе, детерминированно выбирается связь с минимальным `pk`.
