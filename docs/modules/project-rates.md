...
- запрет создания и изменения оценок после дедлайна оценивания программы;
...
- дедлайн `datetime_evaluation_ends` не прошел;
- лимит `max_project_rates` не превышен.

При повторной отправке оценки того же эксперта по тому же критерию значение
обновляется.

Единственный источник дедлайна для legacy-оценок —
`PartnerProgram.datetime_evaluation_ends`. Если значение отсутствует, оценивание
по дедлайну не ограничивается. Запрос разрешен до дедлайна включительно и
отклоняется только при `timezone.now() > datetime_evaluation_ends`. Проверка
относится как к первой оценке, так и к изменению существующей.

Программа для проверки определяется по критериям запроса, поэтому для проекта,
связанного с несколькими программами, применяется дедлайн именно текущей
программы. При закрытом оценивании API возвращает `409 Conflict`:
...
При таком ответе оценки не создаются и не изменяются, а уведомление лидеру
проекта не отправляется.
...
- `datetime_evaluation_ends` блокирует любые legacy-записи `ProjectScore` после
  точного момента дедлайна; отсутствие значения сохраняет оценивание открытым.
