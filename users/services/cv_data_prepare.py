...
class UserCVDataPreparer:
    ...
    def get_prepared_data(self) -> UserCVData:
        ...
        user_data: UserCVData = {
            ...
            "position": user_instance.v2_speciality.name
            if user_instance.v2_speciality
            else user_instance.speciality,
            ...
            "procollab_logo": self._get_encoded_image_from_local_path(
                self.__PROCOLLAB_LOGO_FILENAME
            ),
            "ic_location": self._get_encoded_image_from_local_path(
                self.__IC_LOCATION_FILENAME
            ),
            "encoded_ic_profile": self._get_encoded_image_from_local_path(
                self.__IC_PROFILE_FILENAME
            ),
            "infinity_gradient": self._get_encoded_image_from_local_path(
                self.__INFINITY_GRADIENT_FILENAME
            ),
            # Fonts:
        ...
        user = (
            ...
            .prefetch_related(
                ...
                Prefetch(
                    "skills",
                    queryset=(
                        SkillToObject.objects.select_related("skill").annotate(
                            approuve_count=Count("confirmations")
                        )
                    ),
                ),
                Prefetch(
                    "partner_program_profiles",
                    queryset=PartnerProgramUserProfile.objects.select_related(
                        "partner_program"
                    ),
                ),
            )
