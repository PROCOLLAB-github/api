...
class JwtActivityTrackingTests(APITestCase):
    def setUp(self):
        ...
        self.user = build_user(email=self.email, password=self.password)
    ...
    def test_last_activity_updates_with_throttle(self):
        ...
        self.user.__class__.objects.filter(id=self.user.id).update(last_activity=None)
        ...
        self.user.__class__.objects.filter(id=self.user.id).update(
