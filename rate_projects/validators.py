...
class ProjectScoreValidate:
    ...
    def _validate_quantity_filled_fields(self):
        ...
        if len(filled_fields) > 1:
            raise ValueError(f'Должно быть заполнено менее 2-ух полей!')

        if not isinstance(filled_fields[0], get_type.get(self.criteria_type)):
            raise ValueError(f'Тип введённых данных не совпадает с требуемым!')
    ...
    def _validate_numeric_fields(self):
        ...
        if self.criteria_type != 'bool' and isinstance(added_value, (int, float)):
            if self.criteria_min_value is not None and self.criteria_min_value > added_value:
                raise ValueError("Оценка этого критерия принизила допустимые значения!")
            elif self.criteria_max_value is not None and self.criteria_max_value < added_value:
