...
class DirectTests(TransactionTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.user = get_user_model().objects.create(**TEST_USER1)
    ...
    async def test_send_new_message_to_other_chat(self):  # Message in someone else's chat
        get_user_model().objects.create(**TEST_USER2)
        user = get_user_model().objects.create(**TEST_USER3)
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
    ...
    async def test_read_message_with_new_user(
        ...
        user = get_user_model().objects.create(**TEST_USER2)
        communicator = WebsocketCommunicator(ChatConsumer.as_asgi(), "/ws/chat/")
        communicator.scope["user"] = user
        connected, subprotocol = await communicator.connect(timeout=5)
        data = {
        ...
        await communicator.send_json_to(data)
        response = await communicator.receive_json_from()
        direct_message = DirectChatMessage.objects.get(id=1)
        self.assertFalse("error" in response.keys())
